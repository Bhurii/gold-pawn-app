...
function RenewContent() {
  ...
  async function handleSave() {
    ...
    if (!form.interest && !form.principal_paid) { alert('กรุณาใส่ยอดดอกหรือยอดต้น'); return }

    setSaving(true)
    try {
      const interest = parseFloat(form.interest) || 0
      const principalPaid = parseFloat(form.principal_paid) || 0
      const newAmount = pawn.amount - principalPaid

      if (newAmount <= 0) { alert('ยอดต้นใหม่ต้องมากกว่า 0'); setSaving(false); return }
    ...
    <main className="page-container">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <input className="input-field" type="number" placeholder="เช่น 5000"
            value={form.principal_paid} onChange={e => setForm({ ...form, principal_paid: e.target.value })} />
        </div>
      ...
      <div style={{ marginTop: 24 }}>
        <button className="btn-primary" onClick={handleSave} disabled={saving || newAmount <= 0} style={{ fontSize: 18 }}>
