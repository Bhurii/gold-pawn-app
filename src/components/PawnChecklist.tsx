...
function UploadButtons({ onSelect }: { onSelect: (file: File) => void }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, border: '1.5px dashed var(--border-hover)', borderRadius: 12, padding: '12px', cursor: 'pointer' }}>
        <input type="file" accept="image/*" capture="environment" onChange={(event) => { const file = event.target.files?.[0]; if (file) onSelect(file) }} style={{ display: 'none' }} />
        <span style={{ fontSize: 22 }}>📷</span>
        <span style={{ color: 'var(--gold)', fontSize: 12, fontWeight: 600 }}>ถ่ายรูป</span>
      </label>
      <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, border: '1.5px dashed var(--border-hover)', borderRadius: 12, padding: '12px', cursor: 'pointer' }}>
        <input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) onSelect(file) }} style={{ display: 'none' }} />
      ...
      <CheckRow done={hasTicket} pending={false} label="📄 ตั๋วจำนำ" expanded={expandTicket} onToggle={() => setExpandTicket(!expandTicket)}>
        {hasTicket ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <UploadButtons onSelect={onUploadPawnSlip} />
          </div>
        )}
      </CheckRow>
      ...
      <CheckRow done={hasTransfer && !isPendingTransfer} pending={isPendingTransfer && isOwner} label={isPendingTransfer ? '💸 รอโอนเงิน' : '💸 โอนเงินแล้ว'} expanded={expandTransfer} onToggle={() => setExpandTransfer(!expandTransfer)}>
        {isPendingTransfer && isOwner ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <UploadButtons onSelect={onConfirmTransfer} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          ...
          {interests.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {interests.map((item: any, index: number) => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 8, borderBottom: index < interests.length - 1 ? '0.5px solid var(--border)' : 'none' }}>
                  {item.slip_url ? (
        ...
        <CheckRow done={true} pending={false} label={`🐣 ไถ่ถอนแล้ว · ${toThaiDateShort(redemption.redeem_date)}`} expanded={expandRedeem} onToggle={() => setExpandRedeem(!expandRedeem)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {redemption.pawn_slip_url ? (
              ...
              <div style={{ borderRadius: 10, background: 'var(--black-700)', minHeight: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: 'var(--text-muted)' }}>ยังไม่มีรูปตั๋ว</div>
            )}
            {redemption.transfer_slip_url ? (
              ...
              <div style={{ borderRadius: 10, background: 'var(--black-700)', minHeight: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: 'var(--text-muted)' }}>ยังไม่มีสลิป</div>
            )}
          </div>
        </CheckRow>
      )}
